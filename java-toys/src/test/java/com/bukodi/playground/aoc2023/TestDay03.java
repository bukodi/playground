...
public class TestDay03 {
    ...
    static String example1 = "467..114..\n" +
            "...*......\n" +
            "..35..633.\n" +
            "......#...\n" +
            "617*......\n" +
    ...
    public void testPart1() throws Exception {
        assert procInput(example1) == 4361;
        System.out.println("--------");
        int ss = procInput(challenge);
        System.out.println("Challenge: " + ss);
    }

    char[][] toMap( String input) throws Exception {
        String[] lines = input.split("\n");
        int width = lines[0].length();
        char[][] map = new char[width][lines.length];
        for (int l = 0; l < lines.length; l++) {
            ...
            for( int i = 0; i < width; i++ ) {
                map[i][l] = line.charAt(i);
            }
        }
        return map;
    }
    boolean hasAdjacentSym( char[][] map, int x, int y) throws Exception {
        if( x < 0 || y < 0 || x >= map.length || y >= map[0].length ) {
            throw new Exception("Invalid input: " + x + "," + y);
        }

        String adj = "";
        adj += x>0 && y>0 ? map[x-1][y-1] : "";
        adj += y>0 ? map[x][y-1] : "";
        adj += x<map[0].length-1 && y>0 ? map[x+1][y-1] : "";

        adj += x>0 ? map[x-1][y] : "";
        adj += x<map[0].length-1  ? map[x+1][y] : "";

        adj += x>0 && y<map.length-1 ? map[x-1][y+1] : "";
        adj += y<map.length-1 ? map[x][y+1] : "";
        adj += x<map[0].length-1 && y<map.length-1 ? map[x+1][y+1] : "";

        for( char ch : adj.toCharArray() ) {
    ...
    int procInput( String input) throws Exception {
        char[][] map = toMap(input);
        int[] nums = findNums(map);
        int sum = 0;
        for( int num : nums ) {
            sum += num;
        }
    ...
    int[] findNums( char[][] map) throws Exception {
        List<Integer> nums = new ArrayList<>();

        for( int y = 0; y < map.length; y++ ) {
            System.out.print( new String(map[y]) + ": ");
            int num = 0;
            boolean hasAjacent = false;
            for( int x = 0; x < map[0].length; x++ ) {
                if( Character.isDigit( map[x][y] ) ) {
                    hasAjacent = hasAjacent || hasAdjacentSym(map, x, y);
                    num = num * 10 + (map[x][y] - '0');
                } else {
                    if( hasAjacent && num > 0) {
                        nums.add(num);
    ...
    static String challenge = ".......................153..988....502..842.........588.....441.468......481..........314...715.57............................163..992..512.\n" +
            "............805............*......#.............%...............*........=......%......................#......*.............-....#....*.....\n" +
