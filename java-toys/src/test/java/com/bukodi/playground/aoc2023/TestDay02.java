package com.bukodi.playground.aoc2023;
...
public class TestDay02 {
    ...
    static String example1 = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green\n" +
    ...
    public void testPart1() throws Exception {
        assert procInput(example1,  12, 13,14) == 8;
        System.out.println("--------");
        procInput(challenge, 12,13,14);
        int ss = 59         +61         +67         +68         +79         +92;
        System.out.println("Challenge: " + ss);
    }

    static int procInput( String input, int maxR, int maxG, int maxB) throws Exception {
        possibleLines.clear();
        String[] lines = input.split("\n");
        int sum = 0;
        for (String line : lines) {
            int id = procLine(line, maxR, maxG, maxB);
            sum += id;
        }

        System.out.println("Possible lines: ");
        for( String pline : possibleLines ) {
            System.out.println(pline);
        }
        System.out.println("Summ of IDs: " + sum );
        return sum;
    }

    static List<String> possibleLines = new ArrayList<>();

    static int procLine( String line, int maxR, int maxG, int maxB ) {
        int id = 0;
        ...
        if( header.startsWith("Game") ) {
            id = Integer.parseInt(header.substring(5).trim());
        } else {
            throw new RuntimeException("Invalid input: " + line);
        }
        String[] rounds = line.substring(posColon + 1).split(";");
        int r = 0,g = 0,b = 0;
        for( String round : rounds ) {
            String[] cubes = round.split(",");
            for( String cube : cubes ) {
                String tcube = cube.trim();
                int posSpace = tcube.indexOf(" ");
                if( posSpace == -1 ) {
                    throw new RuntimeException("Invalid input: " + line);
                }
                int num = Integer.parseInt(tcube.substring(0, posSpace));
                if (tcube.endsWith("red") ) {
                    r += num;
                } else if (tcube.endsWith("green") ) {
                    g += num;
                } else if (tcube.endsWith("blue") ) {
                    b += num;
                } else {
                    throw new RuntimeException("Invalid input: " + line);
                }
        ...
        if (r > maxR ) {
            System.out.printf("Invalid RED   %2d > %d : %s\n", r, maxR, line.replace("red", "RED"));
        } else if (g > maxG ) {
            System.out.printf("Invalid GREEN %2d > %d : %s\n", g, maxG, line.replace("green", "GREEN"));
        } else if (b > maxB ) {
            System.out.printf("Invalid BLUE  %2d > %d : %s\n", b, maxB, line.replace("blue", "BLUE"));
        } else {
            System.out.printf("Ok!     Game ID = %-3d : %s\n", id, line);
            possibleLines.add(line);
        }
        return id;
    }

    static String challenge = "Game 1: 4 red, 5 blue, 9 green; 7 green, 7 blue, 3 red; 16 red, 7 blue, 3 green; 11 green, 11 blue, 6 red; 12 red, 14 blue\n" +
